...
func handleSocksConnection(clientConn net.Conn, instances []*tor.Instance, appCfg *config.AppConfig) {
	...
	if err := clientConn.SetReadDeadline(time.Time{}); err != nil { /* Non-fatal */ }
...
func StartSocksProxyServer(ctx context.Context, instances []*tor.Instance, appCfg *config.AppConfig) {
	...
	if err != nil {
		log.Fatalf("SOCKS: Failed to start SOCKS5 proxy on %s: %v", listenAddr, err)
	}
	...
	for {
		...
		if err != nil {
			if strings.Contains(err.Error(), "use of closed network connection") {
				log.Println("SOCKS Proxy: Listener closed.")
				break
			}
