...
func getIntEnv(key string, defaultValue int) int {
	...
	if err != nil {
		log.Printf("Warning: Invalid integer value for ENV %s ('%s'). Using default %d. Error: %v", key, valStr, defaultValue, err)
		return defaultValue
	}
	return val
}
...
func getBoolEnv(key string, defaultValue bool) bool {
	...
	if err != nil {
		log.Printf("Warning: Invalid boolean value for ENV %s ('%s'). Using default %t. Error: %v", key, valStr, defaultValue, err)
		return defaultValue
	}
	return val
}
...
func LoadConfig() *AppConfig {
	once.Do(func() {
		...
		if trimmedAutoRotateIntervalStr == "0" {
			...
			if err == nil && autoRotateSec > 0 {
				...
				if trimmedAutoRotateIntervalStr != "" {
					log.Printf("Config: Invalid AUTO_ROTATE_CIRCUIT_INTERVAL_SECONDS ('%s'). Defaulting to ENABLED with interval %v.", trimmedAutoRotateIntervalStr, cfg.AutoRotateCircuitInterval)
				}
			}
		}
		...
		// Add more detailed logging for other features if desired
	})
	return GlobalConfig
}
