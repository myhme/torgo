...
func Monitor(ctx context.Context, instances []*tor.Instance, appCfg *config.AppConfig) {
	log.Printf("Health monitor started. Check interval: %v", appCfg.HealthCheckInterval)
	if appCfg.HealthCheckInterval <= 0 {
		log.Println("Health monitor disabled (interval <= 0).")
		return
	}
